...
def balance_dataset(input_dir: str, threshold: int) -> None:

    """
    This function balances the dataset by reducing the number of samples in each category 
    to match the category with the fewest samples or a given threshold.

    Args:
        input_dir (str): The directory containing the audio files to process.
        threshold (int): The maximum number of samples allowed in each category.
    ...
    min_count = min(min(genre_counts.values()), threshold)
...
if __name__ == '__main__':
    ...
    # Define the threshold -> OOM
    threshold = 5000
    
    # Call the function to balance the dataset
    balance_dataset(input_dir, threshold)